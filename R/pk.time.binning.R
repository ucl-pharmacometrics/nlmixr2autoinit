...
#' @param dat A data frame containing the pharmacokinetic data with columns for time after dose (tad) and normalised concentration (DVstd).
...
#' @export
#'
pk.time.binning <- function(dat,
                            nbins) {

  test.pool <- dat[dat$EVID == 0, ]
